...
{#if !data.user}
     <!-- Landing-Bereich: wird nur Besuchern ohne Login gezeigt -->
    <div class="flex flex-col items-center justify-center text-center py-24 px-4">
        ...
        <!-- Buttons, die zu Login bzw. Register führen -->
...
<!-- Hinweis, falls noch keine Bilder hochgeladen wurden -->
{#if data.images.length === 0}
    <p class="text-gray-400">No images uploaded yet. Be the first!</p>
{/if}

<!-- Raster mit allen Bildern -->
<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
     {#each data.images as image, i (image.id)}
<!-- Die ersten 3 Bilder (i = 0,1,2) bekommen Gold-/Silber-/Bronze-Rahmen -->
        <a href="/image/{image.id}"
            ...
            <div class="relative">
                <img src={image.image} alt={image.description}
                 ...
                 <!-- Rang-Badge (#1, #2, #3) für die Top-3-Bilder -->
