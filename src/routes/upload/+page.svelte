<script>
    // form enthält eine evtl. Fehlermeldung vom upload-Action.
    let { form } = $props();
</script>

<div class="max-w-lg mx-auto">
    <h1 class="text-3xl font-bold text-gray-800 mb-8">Upload Image</h1>

     <!-- Fehlermeldung anzeigen, falls der Upload fehlgeschlagen ist -->
    {#if form?.error}
    ...
    <!-- Upload-Formular: enctype multipart/form-data wird für Datei-Uploads benötigt -->
    <form action="?/upload" method="POST" enctype="multipart/form-data"
        class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm flex flex-col gap-4">

        <!-- Auswahl der Bilddatei (nur Bilder erlaubt) -->
        <div class="flex flex-col gap-1">
            <label for="image" class="text-sm font-semibold text-gray-700">Image file</label>
